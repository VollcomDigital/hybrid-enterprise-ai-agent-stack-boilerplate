...
.PHONY: dev-mac dev-windows prod down-local down-prod validate-local-mac validate-local-windows validate-prod test-mcp ensure-local-env ensure-prod-env
...
down-local: ensure-local-env
	@$(COMPOSE) --env-file $(LOCAL_ENV) -f $(LOCAL_COMPOSE) down --remove-orphans
