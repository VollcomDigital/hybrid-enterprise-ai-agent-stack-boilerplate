...
dev-mac: ensure-local-env
	...
	OPENCLAW_PRIMARY_MODEL="ollama/$$(awk -F ' *= *' '/^OLLAMA_MODEL[[:space:]]*=/ {print $$2; exit}' $(LOCAL_ENV))" \
...
dev-windows: ensure-local-env
	...
	LOCAL_LLM_MODEL="$$(awk -F ' *= *' '/^VLLM_MODEL[[:space:]]*=/ {print $$2; exit}' $(LOCAL_ENV))" \
	OLLAMA_BASE_URL=http://vllm:8000 \
	OPENCLAW_PRIMARY_MODEL="ollama/$$(awk -F ' *= *' '/^VLLM_MODEL[[:space:]]*=/ {print $$2; exit}' $(LOCAL_ENV))" \
